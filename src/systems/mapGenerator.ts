...
export const regenerateAllMaps = (maps: Record<string, GameMap>, secrets: Secret[]) => {
    ...
    for(let y=0; y<20; y++) {
        for(let x=0; x<20; x++) {
            ...
            maps[id] = generateMap(id, `${name}`, biome, difficulty);
        }
    }
...
export const generateDungeon = (
    ...
    const tierMult = 1 + (worldTier * 0.5);
        ...
        for(let x=1; x<width-1; x++) {
            if (tiles[y][x] === 'FLOOR') {
                 if (Math.random() < 0.08) {
                    ...
                    if (type === 'MAGMA') base = Math.random() > 0.5 ? 'Fire Elemental' : 'Imp';
    ...
    const bossTemplate = MONSTER_TEMPLATES[bossName.replace(/Awakened |Ascended /g, '')] || MONSTER_TEMPLATES['Dragon'];
    const bossHp = Math.floor(bossTemplate.baseHp * Math.pow(SCALE_FACTOR, bossLevel) * tierMult * 1.5);
...
export const generateMap = (id: string, name: string, biome: string, difficulty: number): GameMap => {
    ...
    for(let y=0; y<height; y++) {
        ...
        for(let x=0; x<width; x++) {
            ...
            if (biome === 'DESERT') {
                    ...
                    else if (!isEdge && r > 0.90) row.push('WATER');
                    else row.push('GRASS');
            }
        }
        tiles.push(row);
    ...
    // Always clear center for consistent travel paths in wilderness
    ...
    const maxEnemies = Math.min(8, 2 + Math.floor(difficulty / 2));
    ...
    return { 
        ...
        source: 'mapGenerator.ts (Procedural)'
    };
};
