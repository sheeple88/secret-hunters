...
import { Star, X, Trophy, Lock, Sparkles, BrainCircuit, Shirt, Crown } from 'lucide-react';
...
export const JournalModal: React.FC<JournalModalProps> = ({ gameState, onClose, onTogglePerk }) => {
  const [tab, setTab] = useState<'SECRETS' | 'ACHIEVEMENTS' | 'PERKS' | 'COSMETICS'>('SECRETS');
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/90 flex flex-col p-4 md:p-10 animate-fade-in">
        ...
        {/* Navigation */}
        <div className="flex justify-center mb-4 max-w-5xl mx-auto w-full gap-2 shrink-0 flex-wrap">
            ...
            <button onClick={() => setTab('COSMETICS')} className={`px-4 py-2 rounded-t-lg font-bold flex items-center gap-2 ${tab === 'COSMETICS' ? 'bg-stone-800 text-pink-400' : 'bg-stone-900 text-stone-500 hover:text-stone-300'}`}>
                    ...
                    <div className="bg-purple-900/20 border border-purple-500/30 p-4 rounded-lg shrink-0 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex-1">
                            ...
                            <div className="text-stone-300 italic text-sm min-h-[1.5em]">
                                {isRumorLoading ? ( <span className="animate-pulse">Listening to the spirits...</span> ) : rumor ? ( `"${rumor}"` ) : ( "The spirits know what you seek. Ask them for guidance." )}
                            </div>
                        </div>
                        <button 
            ...
            {tab === 'COSMETICS' && (
                <div className="flex flex-col gap-4 h-full">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2">
                        <h3 className="text-pink-300 font-bold uppercase text-xs tracking-wider mb-2">Unlocked Headwear</h3>
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                            <button 
                                onClick={() => onTogglePerk('COSMETIC_NONE')} // Special handler string, handled in HUD parent or we assume togglePerk handles generic events? Needs update.
                                // Actually, let's just make onTogglePerk generic or add onToggleCosmetic. 
                                // Since props are limited, we'll implement a local emitter or assume App handles it.
                                // For now, simple console log placeholder until App.tsx updates
                                className={`p-4 border-2 rounded bg-stone-800 border-stone-600 flex items-center justify-center hover:bg-stone-700`}
                            >
                                <span className="text-xs text-stone-400">None</span>
                            </button>
                            {gameState.unlockedCosmetics.map(cid => (
                                <button key={cid} className="p-4 border-2 border-pink-500/30 rounded bg-stone-800 flex flex-col items-center justify-center gap-2 hover:bg-stone-750">
                                    <Shirt size={24} className="text-pink-400"/>
                                    <span className="text-xs font-bold text-stone-300 capitalize">{cid.replace('_', ' ')}</span>
                                </button>
                            ))}
                            {gameState.unlockedCosmetics.length === 0 && <div className="col-span-full text-stone-600 italic">No fashion found yet.</div>}
                        </div>
                    </div>
                </div>
            )}
