...
export const UnlockNotification: React.FC<UnlockNotificationProps> = ({ logs }) => {
    ...
    // Effect 1: Watch logs and set notification state
    useEffect(() => {
        if (logs.length === 0) return;
        const lastLog = logs[logs.length - 1];
        
        // Skip if we've already shown this exact log
        if (lastLog.id === lastProcessedLogId.current) return;
        ...
        if (isRecent && (lastLog.type === 'SECRET' || (lastLog.type === 'QUEST' && lastLog.message.includes('Completed')))) {
            lastProcessedLogId.current = lastLog.id;
    ...
    // Effect 2: Handle timer independently so log updates don't cancel it
    useEffect(() => {
        if (notification) {
