...
export default function App() {
  ...
  useEffect(() => {
      // Regenerate Haven's Rest to ensure we have the new 60x50 map
      console.log("Ensuring World Map Integrity...");
      generateHavensRest(MAPS);
      
      // If the map is missing or wrong size (sanity check), regenerate
          ...
          try {
              ...
              if (parsed.gameState && parsed.gameState.stats) {
                  ...
                  setGameState({ 
                      ...
                      equippedCosmetic: gs.equippedCosmetic || 'party_hat'
                  });
              }
  ...
  useEffect(() => {
      if (!hasLoaded) return;
      const interval = setInterval(handleSave, 30000); // Auto save every 30s
  ...
  useEffect(() => {
      const aiInterval = setInterval(() => {
          setGameState(prev => {
              ...
              const aiResult = processEnemyTurn(prev, { ...map, entities: entitiesWithSpawns });
              
              // Update Map
              MAPS[prev.currentMapId].entities = aiResult.updatedEntities;
              ...
              return {
                  ...
                  animations: { ...prev.animations, ...aiResult.animations } 
              };
          });
      }, 800); 
  ...
  const onResetSave = () => {
      localStorage.removeItem('sh_save_v5_fix');
