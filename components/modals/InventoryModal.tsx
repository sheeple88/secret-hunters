...
export const InventoryModal: React.FC<InventoryModalProps> = ({ gameState, playerStats, onClose, onEquip, onConsume }) => {
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4 animate-fade-in">
        <div className="bg-stone-900 w-full max-w-4xl h-[80vh] flex flex-col md:flex-row rounded-xl border-4 border-stone-700 shadow-2xl overflow-hidden">
            ...
            <div className="w-full md:w-1/3 bg-stone-950 p-4 border-b md:border-b-0 md:border-r border-stone-800 flex flex-col gap-4 overflow-y-auto">
                <h2 className="text-xl font-bold text-stone-200 border-b border-stone-800 pb-2 flex justify-between items-center">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm font-mono">
                    <div className="p-2 bg-stone-900 rounded border border-stone-800">
                        <div className="text-stone-500 text-xs uppercase">Strength</div>
                        <div className="text-green-400 text-lg">{formatNumber(playerStats.str)}</div>
                    </div>
                    <div className="p-2 bg-stone-900 rounded border border-stone-800">
                        <div className="text-stone-500 text-xs uppercase">Dexterity</div>
                        <div className="text-green-400 text-lg">{formatNumber(playerStats.dex)}</div>
                    </div>
                    <div className="p-2 bg-stone-900 rounded border border-stone-800">
                        <div className="text-stone-500 text-xs uppercase">Intellect</div>
                        <div className="text-green-400 text-lg">{formatNumber(playerStats.int)}</div>
                    </div>
                    <div className="p-2 bg-stone-900 rounded border border-stone-800">
                        <div className="text-stone-500 text-xs uppercase">Regen</div>
                        <div className="text-green-400 text-lg">{formatNumber(playerStats.regeneration)}</div>
                    </div>
                </div>
