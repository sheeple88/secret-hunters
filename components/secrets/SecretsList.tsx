...
export const SecretsList: React.FC<SecretsListProps> = ({ secrets }) => {
  const unlockedCount = secrets.filter(s => s.unlocked).length;
  
  return (
    <div className="flex flex-col h-full">
        <div className="mb-4 flex items-center justify-between bg-stone-950 p-3 rounded-lg border border-stone-800">
            ...
            <div className="flex items-center gap-2">
                <div className="w-32 h-2 bg-stone-800 rounded-full overflow-hidden">
                    <div className="bg-yellow-500 h-full transition-all duration-1000" style={{ width: `${(unlockedCount / secrets.length) * 100}%` }}/>
                </div>
        ...
        <div className="grid grid-cols-1 gap-3 overflow-y-auto pr-2">
            {secrets.map(secret => {
                const perk = PERKS[secret.perkId];
                return (
                    <div key={secret.id} className={`relative p-4 rounded-lg border-2 transition-all duration-300 group ${secret.unlocked ? 'bg-stone-800 border-yellow-600/50 shadow-lg shadow-yellow-900/10' : 'bg-stone-900 border-stone-800 opacity-80'}`}>
                        <div className="flex justify-between items-start">
                            <div className="flex items-start gap-3">
                                <div className={`mt-1 p-2 rounded-full ${secret.unlocked ? 'bg-yellow-900/30 text-yellow-400' : 'bg-stone-950 text-stone-600'}`}>
                                    {secret.unlocked ? <Unlock size={18} /> : <Lock size={18} />}
                                </div>
                                <div>
                                    <h4 className={`font-bold text-lg leading-none mb-1 ${secret.unlocked ? 'text-yellow-100' : 'text-stone-500'}`}>
